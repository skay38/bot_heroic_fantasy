...
# # # # II) Initialisation variables globals & players # # # #
...
PHRASES_ANNIV=['bon anniversaire', 'joyeux anniversaire', 'bon anniv', 'joyeux anniv']
...
class player:
    def __init__(self, pseudo, lvl, needed_xp, last_message_time):
        self.pseudo = pseudo
        self.lvl = lvl
        self.needed_xp = needed_xp
        self.last_message_time = last_message_time

conn = sqlite3.connect('bot_hf.db')
c = conn.cursor()
try:
    c.execute('''CREATE TABLE PLAYER (pseudo text, lvl real, needed_xp real, last_message_time real)''')
except:
    print("the table already exists")

def init_list_players():
    for row in c.execute('SELECT * FROM PLAYER'):
        LIST_PLAYERS.append(player(row[0], row[1], row[2], row[3]))
...
async def code_names(ctx,espio1, espio2):
    ...
    espion1=bot.get_user(int(espio1[3:-1]))
    espion2=bot.get_user(int(espio2[3:-1]))
    chan=ctx.channel
...
def analyse_anniv(message):
    for k in PHRASES_ANNIV:
