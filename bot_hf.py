...
def carte_alea(n,octave,power):
    h = PerlinNoiseFactory(2,octaves=octave)
    carte=[[0 for i in range(n)] for j in range(n)]
    for i in range(len(carte)):
        for j in range(len(carte)):
            ni=i/len(carte)-0.5
            nj=j/len(carte)-0.5
            e=h(2*ni,2*nj) + 0.5*h(4*ni,4*nj)+0.25*h(8*ni,8*nj)
            carte[i][j]=(e**power).real
    c=np.array(carte,float)
    plt.imshow(c,cmap='gray_r')
    plt.savefig('map.png')

@bot.command()
async def generation_carte(n,octave,power):
    carte_alea(int(n),int(octave),float(power))
    for x in bot.get_all_channels():
        if x.name=="partage-images":
            channel=x
    await bot.send_file(channel,'map.png')
